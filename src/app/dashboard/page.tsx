...
import { 
  ...
  Activity, Trophy, Flame, Crosshair 
} from 'lucide-react';
...
interface AggregatedTelemetry {
  kpis: {
    ...
    mentalStamina: number; // Accuracy in the last 5 questions of a session
...
export default function AnalystDashboard() {
  ...
  useEffect(() => {
    async function fetchAndCompileTelemetry() {
      ...
      // Group attempts by session to calculate sequence numbers (1st question, 2nd question, etc.)
      ...
      Object.values(sessionGroups).forEach((sessionAttempts) => {
        sessionAttempts.forEach((attempt, index) => {
          ...
          // Mental Stamina (Last 5 questions of a standard 19-q test, or index > 13)
          if (index > 13) {
            lateTestTotal++;
            if (isCorrect) lateTestCorrect++;
          }

          // Fatigue/Sequence Mapping
          const seq = index + 1;
      ...
      const cognitiveProfile = Object.entries(wCounts).map(([key, count]) => ({
        name: key, fullDesc: W_CATEGORY_MAP[key], count
      }));

      // Pie Chart Data: Correct vs. Specific Error Types
      ...
      const radarData = radarOrder.map(key => {
        const data = radarMap.get(key) || { correct: 0, total: 1 }; // prevent NaN
        return { subject: key, accuracy: Math.round((data.correct / (data.total || 1)) * 100), fullMark: 100 };
      });

      // Fatigue Sequence mapping (Sort 1 to N)
  ...
  if (!telemetry) return null; // Fallback handled by layout usually
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans p-4 md:p-8 selection:bg-blue-100 pb-24">
      ...
      <header className="max-w-[1400px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        ...
        <div className="flex flex-col items-end gap-4 w-full md:w-auto">
          <AuthBadge />
          <button onClick={() => router.push('/test-initiate')} className="w-full md:w-auto bg-slate-900 text-white px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-slate-800 transition shadow-md">
            Deploy New Diagnostic
          </button>
        </div>
      </header>
      ...
      <main className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Fatigue & Pacing Analysis (Composed) */}
